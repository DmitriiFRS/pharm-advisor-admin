...
const ServiceList: React.FC<Props> = ({ data, page, totalPages }) => {
	...
	const handleDelete = async (id: number) => {
		if (window.confirm("Вы уверены что хотите удалить эту услугу?")) {
			try {
				setIsLoading(true);
				const res = await fetch(`/api/delete/services/${id}`, {
					method: "DELETE",
				});
	...
	return (
		<div className="p-8 max-w-[1600px] mx-auto">
			<div className="flex items-center justify-between mb-6">
				<h1 className="text-24 font-bold text-black-primary">Услуги</h1>
			</div>
			...
			<CommonList
				...
				isLoading={isLoading}
				onEdit={(item) => router.push(`/services/update/${item.id}`)}
